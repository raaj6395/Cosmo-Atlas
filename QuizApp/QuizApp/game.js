...
function navselect(query) {
  // Reset the game state
  game.classList.add('hidden');
  loader.classList.remove('hidden');

  // Fetch questions based on the selected query
  fetchcall(query);
}


window.addEventListener("load",() => fetchcall('space exploration'))


async function fetchcall(query){
  await fetch('https://api.openai.com/v1/chat/completions', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer ' + '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',
  ...
  body: JSON.stringify({
    ...
    'messages': [
      ...
      {
        'role': 'user',
        'content': `make a quiz questions with answers based on ${query} and give atleast 10 questions make it multiple choice questions return it as a result of a request made by a browser inform of a json text which i can parse use this json format while returning
        [
          {
